...
# --- 配置 ---
detfly_root = r"E:\PycharmProject\Datasets\CV\DetFly"
yolo_root = r"E:\PycharmProject\Datasets\CV\DetFly" # YOLO 数据集也存放在同一目录下
source_subdirs = ['010', '020'] # 源图像和标注所在的子目录

images_base_src = os.path.join(detfly_root, "JPEGImages")
annotations_base_src = os.path.join(detfly_root, "Annotations")
...
# delete_original_subdirs = False # 设置为True，在移动完成后尝试删除空的源子目录 -- 不再需要
...
def find_image_annotation_pairs(image_base, annotation_base, subdirs):
    """查找所有图像和对应的标注文件"""
    pairs = []
    print("开始扫描源文件...")
    for subdir in subdirs:
        img_subdir_path = os.path.join(image_base, subdir)
        ann_subdir_path = os.path.join(annotation_base, subdir)
        if not os.path.isdir(img_subdir_path):
            print(f"警告: 图像子目录未找到: {img_subdir_path}")
            continue
        if not os.path.isdir(ann_subdir_path):
             print(f"警告: 标注子目录未找到: {ann_subdir_path}")
             continue

        print(f"扫描目录: {img_subdir_path}")
        # 使用 glob 查找所有 jpg 文件，忽略大小写
        image_files = glob.glob(os.path.join(img_subdir_path, '*.jpg'))

        for img_path in tqdm(image_files, desc=f"扫描 {subdir}"):
            base_filename = os.path.splitext(os.path.basename(img_path))[0]
            ann_path = os.path.join(ann_subdir_path, f"{base_filename}.xml")
    ...
    for img_src_path, ann_src_path in tqdm(pairs, desc=f"处理 {split_name}"):
        ...
        try:
            ...
            # 即使yolo_lines为空，如果XML存在且被解析，也认为label被处理了（生成了空文件）
                 ...
                 converted_count += 1
                 # Optionally print info about empty label file creation
                 # print(f"Info: Created empty label file for {ann_src_path} as no valid objects were found.")
...
if __name__ == "__main__":
    print("开始处理 DetFly 数据集...")

    # 1. 查找所有文件对
    all_pairs = find_image_annotation_pairs(images_base_src, annotations_base_src, source_subdirs)
    ...
    # 5. (可选) 清理空的源子目录 -- 不再需要
    # if delete_original_subdirs:
    #     print("\n开始清理空的源子目录...")
    #     for subdir in source_subdirs:
    #         img_subdir_path = os.path.join(images_base_src, subdir)
    #         ann_subdir_path = os.path.join(annotations_base_src, subdir)
    #         try:
    #             if os.path.isdir(img_subdir_path) and not os.listdir(img_subdir_path):
    #                 os.rmdir(img_subdir_path)
    #                 print(f"已删除空目录: {img_subdir_path}")
    #         except OSError as e:
    #             print(f"删除目录 {img_subdir_path} 失败: {e}")
    #         try:
    #             if os.path.isdir(ann_subdir_path) and not os.listdir(ann_subdir_path):
    #                 os.rmdir(ann_subdir_path)
    #                 print(f"已删除空目录: {ann_subdir_path}")
    #         except OSError as e:
    #             print(f"删除目录 {ann_subdir_path} 失败: {e}")
        # 可以考虑是否也删除 JPEGImages 和 Annotations 根目录（如果它们也空了）
        # try:
        #     if os.path.isdir(images_base_src) and not os.listdir(images_base_src):
        #         os.rmdir(images_base_src)
        #         print(f"已删除空目录: {images_base_src}")
        #     if os.path.isdir(annotations_base_src) and not os.listdir(annotations_base_src):
        #         os.rmdir(annotations_base_src)
        #         print(f"已删除空目录: {annotations_base_src}")
        # except OSError as e:
        #      print(f"删除空的源根目录时出错: {e}")
